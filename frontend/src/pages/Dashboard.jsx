...
const sectionMeta = [
  {
    ...
    label: "Visão geral",
    icon: Wallet,
    description: "Resumo consolidado do financeiro",
  },
  {
    id: "transactions",
  ...
  },
  {
    ...
    label: "Contas",
    icon: Receipt,
    description: "Pagar, receber e recorrência",
  },
  {
    id: "categories",
    label: "Categorias",
    icon: Layers3,
    description: "Organização por contexto",
  },
  {
    id: "reminders",
    label: "Lembretes",
    icon: Bell,
    description: "Alertas e agenda financeira",
  },
  {
    id: "reports",
    label: "Relatórios",
    icon: BarChart3,
    description: "Leituras, projeções e importações",
  },
  {
    id: "ai",
    label: "Assistente Alfred",
    icon: MessageCircle,
    description: "Voz ativa, chat e execução",
  },
];
...
const Dashboard = () => {
  ...
  const submitTransaction = async (event) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Revise os dados do lançamento.",
        variant: "destructive",
      });
    }
  };
  ...
  const submitCategory = async (event) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Confira o nome e tente novamente.",
        variant: "destructive",
      });
    }
  };
  ...
  const submitReminder = async (event) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Não consegui salvar o lembrete.",
        variant: "destructive",
      });
    }
  };
  ...
  const generateRecurringBills = async () => {
    try {
      ...
      toast({
        ...
        description:
          "O Alfred gerou as próximas contas recorrentes dentro da janela prevista.",
      });
      loadAll(currentWorkspace.id);
    } catch (error) {
      toast({
  ...
  const uploadStatement = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Não consegui ler esse extrato agora. Tente CSV, XLSX, XLS ou PDF.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          subtitle={`Visão ${financialView === "general" ? "geral" : financialView === "personal" ? "pessoal" : "empresa"}`}
        />
        <MetricCard
          title="Receitas"
          value={currencyFormatter.format(summary?.income || 0)}
        ...
        <MetricCard
          ...
          subtitle="Andamento operacional"
        />
      </div>
        ...
        <Panel title="Contas e rotina financeira">
          <div className="grid gap-4 md:grid-cols-3">
            <MiniInfo
              ...
              value={summary?.upcoming_bills || 0}
            />
            <MiniInfo
              label="Lembretes ativos"
              value={summary?.active_reminders || 0}
            />
            <MiniInfo
              label="Transações"
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Panel title="Insights do dashboard">
          <InsightList
            items={insights}
            emptyText="Sem alertas principais no momento."
          />
        </Panel>
        <Panel title="Automação e economia">
          <InsightList
            items={automationInsights}
            emptyText="Sem automações sugeridas ainda."
          />
        </Panel>
      </div>
    </div>
      ...
      <Panel title="Histórico financeiro">
        <div className="space-y-3">
          ...
          {transactions.length === 0 && (
            <EmptyState text="Nenhuma transação registrada ainda." />
          )}
        </div>
      </Panel>
    </div>
        ...
        <form onSubmit={submitBill} className="grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Input
              ...
              onChange={(event) =>
                setBillForm({ ...billForm, due_date: event.target.value })
              }
              className={inputClassName}
            />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <select
              ...
              value={billForm.type}
              onChange={(event) =>
                setBillForm({ ...billForm, type: event.target.value })
              }
            >
            ...
            <select
              className={selectClassName}
              value={billForm.account_scope}
              onChange={(event) =>
                setBillForm({ ...billForm, account_scope: event.target.value })
              }
            >
              <option value="business">Empresa</option>
              <option value="personal">Pessoal</option>
          ...
          <Input
            ...
            onChange={(event) =>
              setBillForm({ ...billForm, category: event.target.value })
            }
            className={inputClassName}
          />
          <Input
            ...
            onChange={(event) =>
              setBillForm({ ...billForm, client_name: event.target.value })
            }
            className={inputClassName}
          />
          <Textarea
            ...
            onChange={(event) =>
              setBillForm({ ...billForm, description: event.target.value })
            }
            rows={3}
            className={inputClassName}
          />
          <label className="flex items-center gap-2 text-sm text-slate-300">
            <input
              ...
              onChange={(event) =>
                setBillForm({ ...billForm, recurring: event.target.checked })
              }
            />
            Conta recorrente
          </label>
          {billForm.recurring && (
            <Input
              placeholder="Regra de recorrência"
              value={billForm.recurrence_rule}
              onChange={(event) =>
                setBillForm({
                  ...billForm,
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-400">
                    {bill.type} • {bill.category} • vence em{" "}
              ...
              <div className="mt-4 flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    changeBillStatus(
                      bill.id,
                      bill.type === "payable" ? "paid" : "received",
                    )
                  }
                  className="rounded-full border-white/10 bg-white/[0.03]"
                >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => changeBillStatus(bill.id, "cancelled")}
        ...
        <form onSubmit={submitCategory} className="grid gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <select
              ...
              onChange={(event) =>
                setCategoryForm({ ...categoryForm, kind: event.target.value })
              }
            >
              <option value="expense">Despesa</option>
              <option value="income">Receita</option>
          ...
          <Input
            ...
            onChange={(event) =>
              setCategoryForm({ ...categoryForm, color: event.target.value })
            }
            className="h-11 border-white/10 bg-slate-950/60"
          />
          <Button
            type="submit"
            className="rounded-full bg-cyan-400 text-slate-950 hover:bg-cyan-300"
    ...
    <div className="grid gap-6 xl:grid-cols-[0.88fr_1.12fr]">
      ...
      <Panel title="Agenda financeira">
        <div className="space-y-3">
          ...
          {reminders.length === 0 && (
            <EmptyState text="Nenhum lembrete financeiro criado ainda." />
          )}
        </div>
      </Panel>
    </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap gap-2">
        {["7d", "30d", "90d", "year"].map((period) => (
          <Button
            ...
            className={`rounded-full ${reportPeriod === period ? "border-cyan-400/20 bg-cyan-400 text-slate-950 hover:bg-cyan-300" : "border-white/10 bg-white/[0.03] text-slate-200 hover:bg-white/[0.06]"}`}
          >
            {period}
          </Button>
        ))}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          subtitle={`Período ${report?.period || reportPeriod}`}
        />
        <MetricCard
          title="Contas a pagar"
          value={currencyFormatter.format(report?.payables_open || 0)}
          subtitle="Pendências abertas"
        />
        <MetricCard
          title="Contas a receber"
          value={currencyFormatter.format(report?.receivables_open || 0)}
          ...
          <div className="space-y-3">
            {(report?.top_expenses || []).map((item) => (
              <RecordRow
                key={item.category}
                title={item.category}
                meta="Maior pressão de gasto"
                amount={currencyFormatter.format(item.amount)}
                amountTone="text-rose-300"
              />
            ))}
            {!(report?.top_expenses || []).length && (
              <EmptyState text="Sem despesas suficientes para relatório." />
            )}
          </div>
        </Panel>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        <Panel title="Importação de extrato">
          <div className="space-y-4">
            <p className="text-sm leading-7 text-slate-300">
              Envie CSV, XLS, XLSX ou PDF para o Alfred montar a leitura inicial
              do extrato na visão {scopeLabel.toLowerCase()}.
            </p>
            <Input
              type="file"
            ...
            {statementImportResult && (
              <div className={softCardClassName}>
                <div className="font-medium text-white">
                  {statementImportResult.file_name}
                </div>
                <div className="mt-1 text-sm text-slate-300">
                  Status: {statementImportResult.status}
                </div>
                <div className="text-sm text-slate-400">
                  Linhas detectadas: {statementImportResult.row_count || 0}
                </div>
                {statementImportResult.notes && (
                ...
                <div key={item.id} className={softCardClassName}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <div className="text-xs text-slate-400">
                        {item.file_type} • {item.status} • {item.account_scope}
                      </div>
                    </div>
                    <Badge
                      variant="outline"
      ...
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(8,145,178,0.18),_transparent_25%),linear-gradient(180deg,_#04111d_0%,_#071321_48%,_#020817_100%)] px-6 py-10 text-white">
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          <form
            ...
            className="space-y-3 rounded-[28px] border border-white/10 bg-white/[0.04] p-6 backdrop-blur-xl"
          >
            <Input
              placeholder="Nome da empresa"
              value={workspaceForm.name}
  ...
  return (
    <div className="min-h-screen bg-[#050101] text-slate-200">
      <Sidebar
        items={sectionMeta}
      ...
      <main className="lg:pl-24">
        <div className="mx-auto max-w-[1600px] px-4 pb-10 pt-20 sm:px-6 lg:px-8 lg:pt-8 relative">
          {/* Objeto Central com a sombra vermelha irradiando */}
          <div className="relative rounded-[34px] border border-primary/20 bg-black/40 px-5 py-5 shadow-[0_0_120px_-15px_rgba(221,18,34,0.2)] backdrop-blur-2xl">
            <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
              <div className="space-y-3">
                <Badge
                  variant="outline"
                  className="rounded-full border-primary/30 bg-primary/10 px-3 py-1 text-[11px] uppercase tracking-[0.24em] text-primary"
                >
                  Workspace ativo
                </Badge>
                <div>
              ...
              <div className="space-y-3 xl:text-right">
                  ...
                  {scopeOptions.map((scope) => (
                    <Button
                      ...
                      className={`rounded-full px-4 transition-all duration-300 ${
                        financialView === scope.id
                          ? "border-primary bg-primary text-white shadow-[0_0_15px_rgba(221,18,34,0.4)]"
                ...
                <div className="flex flex-wrap gap-2 xl:justify-end">
                  <Badge
                    variant="outline"
                    className="rounded-full border-white/5 bg-white/5 px-3 py-1 text-slate-300"
                  >
                  ...
                  <Badge
                    variant="outline"
                    className="rounded-full border-white/5 bg-white/5 px-3 py-1 text-slate-400"
                  >
            ...
            <div className="mt-5 border-t border-primary/10 pt-5">
              {loading ? (
                <div className="rounded-[24px] border border-white/5 bg-black/20 p-8 text-center text-slate-500">
...
/* Subcomponentes com estética escura e translúcida */

const Panel = ({ title, action, children }) => (
  <Card className="rounded-[30px] border-white/5 bg-black/20 p-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl hover:border-primary/20 transition-colors duration-300">
...
const MetricCard = ({ title, value, subtitle }) => (
  <Card className="rounded-[28px] border-white/5 bg-black/20 p-5 shadow-[0_8px_30px_rgba(0,0,0,0.5)] backdrop-blur-xl hover:border-primary/20 transition-colors duration-300">
...
const RecordRow = ({ title, meta, amount, amountTone = "text-white" }) => (
  <div className="rounded-[24px] border border-white/5 bg-white/[0.02] px-4 py-4 hover:bg-white/[0.04] transition-colors">
...
const EmptyState = ({ text }) => (
  <div className="rounded-[24px] border border-dashed border-white/10 bg-black/10 p-6 text-sm text-slate-500 text-center">
    {text}
  </div>
);
  ...
  return (
    <div className="space-y-3">
      {items.map((item, index) => (
        <div
          key={index}
          className="rounded-[24px] border border-white/5 bg-white/[0.02] p-4 hover:border-primary/30 transition-colors duration-300"
        >
          <div className="font-medium text-white">
            {item.label || item.type || "Insight"}
          </div>
